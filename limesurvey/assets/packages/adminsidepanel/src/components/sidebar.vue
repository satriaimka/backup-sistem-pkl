...
export default {
  ...
  computed: {
    useMobileView() {
      return false;
    }, // DISABLED: Force desktop view for mobile sidebar functionality
    isActive() {
    ...
    showSideMenu() {
      return !this.$store.getters.isCollapsed && this.currentTab == "settings";
    },
    showQuestionTree() {
      return (
        !this.$store.getters.isCollapsed && this.currentTab == "questiontree"
      );
    },
    ...
    controlActiveLink() {
      ...
      LS.ld.each(this.questiongroups, (itm, i) => {
        let regTest = new RegExp(
          "questionGroupsAdministration/view\\?surveyid=\\d*&gid=" +
            itm.gid +
            "|questionGroupsAdministration/edit\\?surveyid=\\d*&gid=" +
            itm.gid +
            "|questionGroupsAdministration/view/surveyid/\\d*/gid/" +
            itm.gid +
            "|questionGroupsAdministration/edit/surveyid/\\d*/gid/" +
      ...
      let questionId = document.querySelector(
        '#edit-question-form [name="question[qid]"]'
      );
      if (questionId !== null) {
        ...
        LS.ld.each(this.questiongroups, (itm, i) => {
          LS.ld.each(itm.questions, (itmm, j) => {
            lastQuestionObject =
              questionId === itmm.qid ? itmm : lastQuestionObject;
...
<template>
  <div
    ...
    :style="{
      'max-height': $store.state.inSurveyViewHeight,
      display: hiddenStateToggleDisplay,
    }"
  ...
  >
    <template v-if="(useMobileView && smallScreenHidden) || !useMobileView">
      ...
      >
        <div
          class="ls-flex ls-flex-column fill align-content-center align-items-center"
        >
          <i class="ri-loader-2-fill remix-2x remix-spin"></i>
        </div>
      </div>
      <div class="col-12 mainContentContainer" key="mainContentContainer">
    ...
    <div
      class="scoped-placeholder-greyed-area"
